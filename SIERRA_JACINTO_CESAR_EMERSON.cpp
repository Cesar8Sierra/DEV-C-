...
void registroDoc(){
    if(totalDocentes<10){
        ...
        if(Doc[totalDocentes].DNI_Doc.length()!=8){
        	cout<<"||*DNI NO ACEPTABLE."<<endl;
        	goto error;
		}
...
void agregarEst(int indiceDocente){
    int cant;
    ...
    for(int i=Doc[indiceDocente].Cant_Est; i<Doc[indiceDocente].Cant_Est+cant; i++){
        ...
        if(Doc[indiceDocente].Est[i].DNI_Est.length()!=8){
        	cout<<"DNI NO ACEPTABLE."<<endl;
        	goto error_est;
		}
...
void modificarDatos(int indice_Doc){
    ...
    for(int j=0; j<Doc[indice_Doc].Cant_Est && flagg==0; j++){
...
int main(){
    ...
    do{
	    ...
	    cout<<"||4. MOSTRAR TODOS LOS DATOS    ||\n";
	    cout<<"||5. SALIR                      ||\n";
	    cout<<"	OPCION: ";
	    cin>>opcion;
	    system("cls");
